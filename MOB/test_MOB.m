...
train_data = xlsread('MoB_data.xls');        % 读入train数据
if exist('result.xls','file')                % 文件存在就删除，防止上次的结果影响
    delete result.xls;
end
init_phi = 0.1:0.1:0.9;                      % phi初始值从0.1到0.9
for i = 1:length(init_phi)
     MOB(train_data, init_phi(i), (i-1)*2);  % 建议 help MOG
end

plot_res = xlsread('result.xls');            % 读入result数据    
figure('NumberTitle', 'off', 'Name', 'MOB'); hold on; grid on;
title('\phi从0.1到0.9'); xlabel('迭代次数k'), ylabel('\phi初始值'); 
